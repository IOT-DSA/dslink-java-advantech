...
public class AdvantechDevice {
	...
	AdvantechDevice(AdvantechPort port, JsonObject json) {
		...
		node.setAttribute("_dstype", new Value("device"));
	...
	AdvantechDevice(AdvantechPort port, Node node) {
		this.port = port;
		this.node = node;
	...
	void init() {
		...
		try {
			...
			if (response != null) {
				JsonObject details = (JsonObject) new JsonObject(response).get("Device");
